...
- **Semantic Regex Strategy**: Scanning AsciiDoc text uses a **three-pass approach** to minimize false positives. (1) Multi-letter extensions (Z/S-prefixed like `Zba`, `Svnapot`) are matched broadly since they are unambiguous. (2) Single-letter extensions (`I`, `M`, `A`, `F`, etc.) are only detected when they appear in ISA context (e.g., "M extension" or "M-extension"), avoiding false matches from regular English words. (3) Full ISA strings like `RV32IMAFD` are decomposed into individual extensions. A blocklist further filters common English words (like "Some" or "Sign") caught by the S-prefix pattern.
